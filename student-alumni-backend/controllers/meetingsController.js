...
exports.requestMeeting = async (req, res) => {
  try {
    ...
    const alumniDoc = await Alumni.findOne({
      userId: meetingObj.alumniId && meetingObj.alumniId._id,
    }).select("_id");
    const studentDoc = await Student.findOne({
      userId: meetingObj.studentId && meetingObj.studentId._id,
    }).select("_id");
  ...
  try {
    ...
    const meetingsWithDocs = await Promise.all(
      meetings.map(async (m) => {
        const mo = m.toObject();
        const alumniDoc = await Alumni.findOne({
          userId: mo.alumniId && mo.alumniId._id,
        }).select("_id");
        const studentDoc = await Student.findOne({
          userId: mo.studentId && mo.studentId._id,
        }).select("_id");
  ...
  try {
    ...
    try {
      if (
        ...
        const meetLink = await googleMeetService.createMeeting(
          `Meeting with ${meeting.studentId || "Student"}`,
          meeting.start,
          meeting.end
        );
      ...
      console.warn(
        "googleMeetService.createMeeting failed:",
        gErr && gErr.message ? gErr.message : gErr
      );
    }
    ...
    try {
      if (
        ...
        const alumniUser = await Alumni.findOne({
        ...
        const ev = await createMeetEvent({
          summary: `Meeting: ${
            studentUser && studentUser.email ? studentUser.email : "Student"
          }`,
          start: meeting.start,
          end: meeting.end,
          attendees,
...
exports.cancelMeeting = async (req, res) => {
  try {
    ...
    if (!meeting) return res.status(404).json({ message: "Meeting not found" });
    if (
      String(meeting.alumniId) !== String(userId) &&
      String(meeting.studentId) !== String(userId)
    )
      return res.status(403).json({ message: "Not authorized" });
  ...
  try {
    ...
    if (String(meeting.alumniId) === String(userId)) {
      ...
      if (!slotExists)
        return res
          .status(400)
          .json({ message: "Proposed slot is not in your availability" });
    }

    meeting.proposedStart = startDt;
    meeting.proposedEnd = endDt;
    meeting.proposer =
      String(meeting.alumniId) === String(userId) ? "alumni" : "student";
