...
const StudentList = () => {
  ...
  const submitReport = async () => {
    ...
    try {
      ...
      const reporterId = currentUser?._id || JSON.parse(localStorage.getItem("user") || "null")?._id || null;
      const url = API ? `${API}/reports` : "/api/reports";
      const body = { reportedUserId: reportTargetId, reporterId, reason: reportReason, description: reportDescription };
    ...
    finally { setReportSubmitting(false); setReportModalOpen(false); setReportTargetId(null); }
  };
  ...
  const handleBlock = async (id) => {
    ...
    try {
      ...
      setStudents(prev => prev.map(u => (u._id === id ? { ...u, allowed: false } : u)));
      alert("User blocked");
    } catch (err) { console.error(err); alert(err.message || "Failed to block user"); }
          ...
          return (
            <div className="alumni-card" key={s._id}>
              <div className="card-menu-wrapper">
                <button className="dots-btn" aria-label="More actions" onClick={(e) => { e.stopPropagation(); setOpenMenuId(openMenuId === s._id ? null : s._id); }}>
                  ⋮
                </button>
                {openMenuId === s._id && (
                  <div className="card-menu" onClick={(e) => e.stopPropagation()}>
                    {currentUser && currentUser.role === 'alumni' && (
                      <button className="menu-item" onClick={() => { setOpenMenuId(null); navigate(`/alumni/student/${s._id}/request`); }}>
                        Request Meeting
                      </button>
                    )}
                    <button className="menu-item" onClick={() => { setOpenMenuId(null); navigate(`/chats/${s._id}`); }}>
                      Chat
                    </button>
                    <button className="menu-item" onClick={() => handleBlock(s._id)}>Block</button>
                    <button className="menu-item" onClick={() => handleReport(s._id)}>Report</button>
                  </div>
                )}
              </div>
      ...
      {reportModalOpen && (
        <div className="report-modal-overlay">
          <div className="report-modal" role="dialog" aria-modal="true">
            ...
            <label>
              Description
              <textarea value={reportDescription} onChange={(e) => setReportDescription(e.target.value)} rows={4} placeholder="Short description (what happened)"></textarea>
            </label>

            <div style={{ marginTop: 12, textAlign: "right" }}>
              <button onClick={() => setReportModalOpen(false)} disabled={reportSubmitting} style={{ marginRight: 8 }}>Cancel</button>
              <button onClick={submitReport} disabled={reportSubmitting}>{reportSubmitting ? "Submitting..." : "Submit Report"}</button>
            </div>
          </div>
        </div>
      )}
